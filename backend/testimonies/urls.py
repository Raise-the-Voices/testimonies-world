...
def _gate_ui(view):
    """Login-required in production, open in DEBUG.

    The interactive Swagger UI and Redoc let a visitor browse every
    schema-visible field on every endpoint. Worth protecting in prod,
    not worth a login redirect loop when a dev is running locally.
    The raw schema at /api/schema/ stays public either way so the
    frontend's `npm run gen:api` (orval) works in CI without auth.
    """
...
urlpatterns = [
    ...
    # types + Zod schemas (see frontend/orval.config.ts). The raw
    # schema stays PUBLIC so CI can pull it without credentials.
    #
    # The interactive swagger-ui and redoc views are gated by
    # `_gate_ui` — open in DEBUG (no redirect loop for local devs),
    # login_required otherwise. They also pull from a CDN-sidecar
    # bundle (drf_spectacular_sidecar) which is heavier than the
    # raw schema, so prod visitors get redirected to login rather
    # than pulling the sidecar bundle anonymously.
    path('api/schema/', SpectacularAPIView.as_view(), name='schema'),
    path(
        'api/schema/swagger-ui/',
        _gate_ui(SpectacularSwaggerView.as_view(url_name='schema')),
        name='swagger-ui',
    ),
    path(
        'api/schema/redoc/',
        _gate_ui(SpectacularRedocView.as_view(url_name='schema')),
        name='redoc',
    ),
