...
# Slug auto-generation: a UUID-prefixed default. Module-level so
# tests can monkeypatch it if they need a deterministic value.
import uuid
...
class TestimonialWriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            # Slug is auto-generated by Testimonial.save() when blank;
            # here we just tell DRF not to require it on POST. A
            # frontend PATCH can supply one to override.
    ...
    def create(self, validated_data):
        # Slug is required by the model (unique=True) but tedious for
        # volunteers to invent. Derive from the row's translation_group
        # UUID — already auto-generated per model save — so siblings
        # in different languages keep distinct slugs while a single-
        # language testimonial still gets a stable unique slug.
        # Frontend can still PATCH a custom slug in afterwards.
        validated_data.setdefault(
            'slug',
            f't-{uuid.uuid4().hex[:12]}',
        )
